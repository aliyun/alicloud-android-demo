...
class InfoFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 使用Config类的AppKey和AppSecret，如果为空则从MetaData获取
        binding.tvAppKey.text = if (!TextUtils.isEmpty(Config.APP_KEY)) {
            Config.APP_KEY
        } else {
            requireContext().getAppMetaData("com.alibaba.app.appkey")
        }
        binding.tvAppSecret.text = if (!TextUtils.isEmpty(Config.APP_SECRET)) {
            Config.APP_SECRET
        } else {
            requireContext().getAppMetaData("com.alibaba.app.appsecret")
        }
