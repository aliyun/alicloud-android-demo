 ...
 * @date 2024/07/22
 */
class CaseAdapter(val data:List<Pair<Int , String>>):RecyclerView.Adapter<CaseAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, position: Int): ViewHolder {
        return ViewHolder(CaseItemBinding.inflate(LayoutInflater.from(parent.context),parent , false))
    }
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.itemView.setBackgroundResource(if (position == selectItem) {
            R.drawable.httpdns_case_item_bg_sel} else R.drawable.httpdns_case_item_bg_def)

        (viewHolder.itemView as TextView).setTextColor(if (position == selectItem){
